...
sys.path.append("./")
from pose_gaussian.utils.ct_utils import get_geometry_tigre, recon_volume


## helper functions for perturbation
def generate_dR(length, args):
    rand_r = torch.normal(mean = 0.0, std=torch.ones((length, 3)) * args.std_rot)
    R = SO3.exp(rand_r)
    rot_matrix = R.matrix()[:, :3, :3].numpy()

    return rot_matrix

def generate_dt(length, args):
    return np.random.normal(loc = 0.0, scale = args.std_trans, size = (length, 3))

def transform_volume(vol, dR, dt, idx):
    w, h, d = vol.shape
    ...
    centeroid = np.mean(coords, axis = 1, keepdims=True)
    centered = coords - centeroid
    transformed = rotation @ centered + centeroid + translation[:, None]

    x_, y_, z_ = transformed.reshape(3, w, h, d)
    vol_transformed = scipy.ndimage.map_coordinates(vol, [x_, y_, z_])
    return vol_transformed[:, :, ::-1]
...
def main(args):
    ...
    ############################### modified ###############################
    if args.std_rot > 0.0:
        dR = generate_dR(projs_train_angles.shape[0], args)
    else:
        dR = None
    
    if args.std_trans > 0.0:
        dt = generate_dt(projs_train_angles.shape[0], args)
    else:
        dt = None
    ...
    # dictionaries to save parameter noise
    quat_dict = dict()
    trans_dict = dict()
    ...
    meta = {
        ...
        "std_rot": args.std_rot,
        "std_trans": args.std_trans,
    }

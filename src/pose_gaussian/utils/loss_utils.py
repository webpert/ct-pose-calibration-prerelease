...
def tv_2d_loss(vol, reduction="sum"):
    ...
    if reduction == "mean":
        total_elements = (
            (vol.shape[0] - 1) * vol.shape[1]
            + vol.shape[0] * (vol.shape[1] - 1)
        )
        tv = tv / total_elements
    return tv
...
def l2_trunc_loss(network_output, gt, alpha=0.5):
    diff = network_output - gt
    abs_diff = torch.abs(diff)

    l2_mask = abs_diff < alpha
    l2_loss = (diff[l2_mask] ** 2).mean() if l2_mask.any() else torch.tensor(0.0, device=network_output.device)

    trunc_mask = ~l2_mask
    trunc_loss = (alpha ** 2) * trunc_mask.float().mean()

    total_loss = l2_loss + trunc_loss

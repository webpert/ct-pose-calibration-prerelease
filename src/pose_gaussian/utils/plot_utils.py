...
def show_two_slice(
    ...
    if torch.is_tensor(slice2):
        slice2 = t2a(slice2)
    if not no_diff:
        # diff = np.abs(slice1 - slice2) ** gamma   # kschoi
        diff = (slice1 - slice2) ** gamma   # kschoi
    ...
    if not no_diff:
        # img3 = ax[2].imshow(diff, cmap=cmap, vmin=0.0)    # kschoi
        img3 = ax[2].imshow(diff, cmap='bwr', vmin=-np.max(np.abs(diff)), vmax=np.max(np.abs(diff)))    # kschoi
...
def show_histogram(input_data, means):    # both arguments are numpy array
    # Visualization
    data = input_data.flatten()

    matplotlib.use("TkAgg")

    plt.hist(data, bins=100, density=True, alpha=0.6, color='gray', label='Data histogram')
    # Mark means
    for mean in means:
